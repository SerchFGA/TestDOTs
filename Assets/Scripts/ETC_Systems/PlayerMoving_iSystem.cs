...
public partial struct PlayerMoving_iSystem : ISystem
{
    ...
    {
        // state.RequireForUpdate<PlayerInput_System>();
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        if (shouldMove.up)
        {
            //playerTransform.TranslateLocal(new float3(0f, 0f, deltaTime*10));
            rb.ApplyLinearImpulseLocalSpace(new float3(0f,0f,1));
            GameManager_Script.Instance.spaceShipON();
        }

        //Rotate Right
        if (shouldMove.right)
        {
            //playerTransform.RotateWorld(Quaternion.Euler(0f, deltaTime*10, 0f));
            rb.ApplyAngularImpulseLocalSpace(new float3(0f, 0.02f, 0f));
        }
